...
npm test           # astro build && vitest run (unit + 26-test e2e suite)
...
- `worker/` — platform-independent shared modules: `storage.js` (R2 layout
  `artifacts/{uuid}/v{n}.html` + `meta.json`), `auth.js` (timing-safe token),
  `ratelimit.js` (`RateLimiterDO`, fixed 60s window, IPv6 /64 bucketing),
...
- Rate limiting: read scope on pages/raw/API GETs (HEAD counts as GET),
  upload scope on `POST /api/artifacts`; 404s, `/healthz`, and deletes are
  unlimited. Limiter runs before auth on POST.
- Baseline security headers are applied only where a route hasn't set the
  header; `Access-Control-Allow-Origin: *` is unconditional.
...
- `security: { checkOrigin: false }` is deliberate: bearer-token API with
  CORS `*`, no cookie auth — Astro's origin check would 403 every
  non-browser POST.
